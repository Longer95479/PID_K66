...
{ 	
    while(!(UART_S1_REG(UARTN[4]) & UART_S1_TDRE_MASK));
    //发送数据
...
void UART_Init (UART_Type * uratn, uint32 baud)
{
    ...
    //写 SBR
    temp = UART_BDH_REG(uratn) & (~UART_BDH_SBR_MASK);           //缓存 清空 SBR 的 UARTx_BDH的值
    UART_BDH_REG(uratn) = temp |  UART_BDH_SBR(sbr >> 8);        //先写入SBR高位
    UART_BDL_REG(uratn) = UART_BDL_SBR(sbr);                     //再写入SBR低位

    //写 BRFD
    temp = UART_C4_REG(uratn) & (~UART_C4_BRFA_MASK) ;           //缓存 清空 BRFA 的 UARTx_C4 的值
